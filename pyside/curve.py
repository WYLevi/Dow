    ...
    def createBackground(self, device, frame):
        ...
        while True:
            ...
            if len(xy) != 0:
                ...
                showFrame = cv2.putText(showFrame, str(round(abs((self.maxPixel[0]- self.detecetHeight) * self.pixeltomm) - self.buffer, 2)) + 'mm', (x - 100, y ), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 1, cv2.LINE_AA)
            ...
            #無白點狀況
                ...
                #無白點高度顯示為None
                showFrame = cv2.putText(showFrame, 'None', (x - 100, y), cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 255, 255), 1, cv2.LINE_AA)
                #print('normal')
            ...
            if 0 < len(self.X) < self.substrateLength or len(xy) != 0:
                #大於指定閥值顯示異常
                if (round(abs((self.maxPixel[0]- self.detecetHeight) * self.pixeltomm) - self.buffer ,2)) > self.NGThreshold + self.buffer:
                    self.result = True
                    self.detecet = True
                    showFrame = cv2.putText(showFrame, 'Warning! deviation:{}mm'.format(str(round(abs((self.maxPixel[0]- self.detecetHeight) * self.pixeltomm) - self.buffer ,2))), (50, 50 ), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)
                    #記錄該基板最高點影像
